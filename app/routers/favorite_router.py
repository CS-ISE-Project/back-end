...
from app.controllers.favorite_controller import add_favorite_controller, delete_favorite_controller, get_user_favorites

auth_scheme=HTTPBearer()
router = APIRouter()

@router.get("/", response_model=list[int])
def read_user_favorites(db: Session = Depends(get_db), token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
    verify_token(token.credentials, 'user')
    return get_user_favorites(token.credentials, db)
